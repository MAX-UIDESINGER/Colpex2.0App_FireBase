...
import com.example.colpex20app_firebase.Adactadores.AdacterBlanqueado.ItemAdacterBlanqueado;
...
public class FragmentBlanqueadoTrabajo extends Fragment implements RvItemClick {
    // texview Blanqueado
    ...
    //Item de Lote
    private List<ItemBlanqueado> itemList = new ArrayList<>();
    ItemAdacterBlanqueado adapter;
    //Contendio
    List<ClaseBlanqueado> subItemListBlanqueado = new ArrayList<>();
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        ...
        return inflater.inflate(R.layout.fragment_blanqueado_trabajo, container, false);
    }
    ...
    private void DatosItemBlanqueado() {
        adapter = new ItemAdacterBlanqueado(itemList, this) {
            @Override
    ...
    private void filterData(String query) {
        List<ItemBlanqueado> filteredItemList = new ArrayList<>();
        for (ItemBlanqueado item : itemList) {
            String itemTitleLote = item.getItemTitleLote().toLowerCase();
            if (itemTitleLote.contains(query.toLowerCase())) {
                filteredItemList.add(item);
            } else {
                List<ClaseBlanqueado> subItemListBlanqueado = item.getSubItemListBlanqueado();
                ArrayList<ClaseBlanqueado> filteredSubItemList = new ArrayList<>();
                for (ClaseBlanqueado dato : subItemListBlanqueado) {
                    ...
                    if (dato.getNumeroLote().toLowerCase().contains(searchValue) ||
                            ...
                            dato.getCarga().toLowerCase().contains(searchValue) ||
                            dato.getVacio().toLowerCase().contains(searchValue) ||
                            dato.getTempTrabajo().toLowerCase().contains(searchValue) ||
                            dato.getArcilla().toLowerCase().contains(searchValue) ||
                            dato.getALLote().toLowerCase().contains(searchValue) ||
                            dato.getCarbonActivado().toLowerCase().contains(searchValue) ||
                            dato.getCALote().toLowerCase().contains(searchValue) ||
                            dato.getSilice().toLowerCase().contains(searchValue) ||
                            dato.getSLote().toLowerCase().contains(searchValue) ||
                            dato.getFFAFinal().toLowerCase().contains(searchValue) ||
                            dato.getColorFinal().toLowerCase().contains(searchValue) ||
                            dato.getAndisidinaFinal().toLowerCase().contains(searchValue) ||
                ...
                if (!filteredSubItemList.isEmpty()) {
                    ItemBlanqueado filteredItem = new ItemBlanqueado(itemTitleLote, filteredSubItemList);
                    filteredItemList.add(filteredItem);
                }
            }
        }
        ItemAdacterBlanqueado adapter = new ItemAdacterBlanqueado(filteredItemList,this) {
            @Override
            public void onDataChanged() {
                if (progressBar != null) {
                    progressBar.setVisibility(View.GONE);
    ...
    private void inserccion(ClaseBlanqueado claseBlanqueado, String reactor, String carga, String vacio, String tempTrabajo, String arcilla, String ALLote, String carbonActivado, String CALote, String silice, String SLote, String FFAFinal, String colorFinal, String andisidinaFinal) {
        ...
        databaseReference.child("Blanqueado").child(claseBlanqueado.getNumeroLote()).child(claseBlanqueado.getId()).setValue(b);
            ...
            databaseReference.child("Blanqueado").addValueEventListener(new ValueEventListener() {
                ...
                public void onDataChange(@NonNull DataSnapshot snapshot) {
                    ...
                    for (DataSnapshot dataSnapshot : snapshot.getChildren()) {
                        ...
                        subItemListBlanqueado = new ArrayList<>();
                        for (DataSnapshot subSnapshot : dataSnapshot.getChildren()) {
                            ClaseBlanqueado dato = subSnapshot.getValue(ClaseBlanqueado.class);
                            subItemListBlanqueado.add(dato);
                        }
                        ItemBlanqueado item = new ItemBlanqueado(itemTitleLote, subItemListBlanqueado);
                        progressBar.setVisibility(View.GONE);
